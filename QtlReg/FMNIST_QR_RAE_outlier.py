...
def beta_loss_function(recon_x, x,mu,beta,Q):
    ...
    if beta > 0:
        ...
        MSE = torch.sum(torch.mean(torch.max(Q * (recon_x.view(-1, 784) - x.view(-1, 784)), (Q - 1) * (recon_x.view(-1, 784) - x.view(-1, 784))),(1)))

    # compute KL divergence

   
    zterm = torch.mean(mu**2)
    z_var=1000/9
    return MSE+(1/z_var)*zterm
##########################################
...
def train(epoch, beta_val):
    ...
    for batch_idx, (data, data_lab) in enumerate(train_loader):
        ...
        loss_Q1 = beta_loss_function(recon_batch[:,0,:], data, mu, beta_val,0.15)
        loss_Q2= beta_loss_function(recon_batch[:,1,:], data, mu,beta_val,0.5)
        loss_Q3 = beta_loss_function(recon_batch[:,2,:], data, mu ,beta_val, 0.85)

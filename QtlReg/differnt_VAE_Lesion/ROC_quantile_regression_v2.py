...
model_median = model=UNet(3, 9).cuda()


model_median.load_state_dict(torch.load('results/VAE/VAE_QR_brain_195.pth'))
...
with torch.no_grad():
    ...
    for i, data in enumerate(tqdm(in_data)):
        ...
        mu, logvar, rec = model_median(data)
        msk = torch.tensor(data> 1e-6).float()
        out_median[i, ] = rec[:,3:6,:,:]*msk
        out_Q1[i, ] = rec[:,0:3,:,:]*msk
        out_Q2[i, ] = rec[:,6:9,:,:]*msk
        out_std[i, ] = (torch.abs(rec[:,6:9,:,:] - rec[:,0:3,:,:]) / 2.0) *msk
...
save_image(comparison,
           'results/VAE/recon_QR_pval_brain.png',
           nrow=16,
           scale_each=False,
           normalize=True,
           range=(0, 1))

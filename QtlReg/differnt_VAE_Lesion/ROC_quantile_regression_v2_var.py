...
with torch.no_grad():
    ...
    for i, data in enumerate(tqdm(in_data)):
        ...
        log_var[i, ] = rec[:,3:6,:,:]*msk
        out_median[i, ] = rec[:,0:3,:,:]*msk
        out_std[i, ] =  rec[:,3:6,:,:].mul(0.5).exp_() *msk
...
n = np.array(range(16))
comparison = torch.cat([
    in_data[n, 2:3], out_median[n, 2:3], out_std[n, 2:3],
    sig_msk[n, 2:3],
])

...
with torch.no_grad():
    ...
    for i, data in enumerate(tqdm(in_data)):
        ...
        rec=torch.reshape(rec, (-1, 2,784))
        out_mean[i, ] = rec[:,1,:].view(1, 28, 28).cpu()
        out_std[i, ] = (rec[:,1,:]-rec[:,0,:]).view(1, 28, 28).cpu() 
...
comparison = torch.cat([
    ...
    abs(in_data[n:n*2] - out_mean[n:n*2]), out_std[n:n*2], z_score[n:n*2] / 10,

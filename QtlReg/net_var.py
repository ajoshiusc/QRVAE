...
class VAE(nn.Module):
    def __init__(self, zsize, layer_count=3, channels=3):
        ...
        setattr(self, "deconv%d_var" % (self.layer_count + 1), nn.ConvTranspose2d(inputs, channels, 4, 2, 1))
    ...
    def decode(self, x):
        ...
        x_mean = F.tanh(getattr(self, "deconv%d" % (self.layer_count + 1))(x))
        var=F.sigmoid(getattr(self, "deconv%d_var" % (self.layer_count + 1))(x))
        return x_mean,var
    ...
    def forward(self, x):
        ...
        mean_x,mean_var=self.decode(z.view(-1, self.zsize, 1, 1))
        return mean_x,mean_var, mu, logvar

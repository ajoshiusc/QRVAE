...
class Decoder(nn.Module):
    def __init__(self, input_size, representation_size):
        ...
        self.act2 = nn.Sequential(nn.BatchNorm2d(128),
                                  nn.ReLU())
            # 128 x 64 x 64
        self.deconv3 = nn.ConvTranspose2d(128, 64, 5, stride=2, padding=2)
        self.act3 = nn.Sequential(nn.BatchNorm2d(32),
                                  nn.ReLU())
            # 32 x 128 x 128
        self.deconv4 = nn.ConvTranspose2d(64, 3, 5, stride=1, padding=2)
        self.deconv5 = nn.ConvTranspose2d(64, 6, 5, stride=1, padding=2)
            # 3 x 128 x 128
    ...
    def forward(self, code):
        ...
        output = self.deconv5(output, output_size=(bs, 6, 64, 64))
...
class Encoder_st(nn.Module):
    def __init__(self, input_channels, output_channels, representation_size = 64):
        super(Encoder_st, self).__init__()
        # input parameters
        self.input_channels = input_channels
        self.output_channels = output_channels
        ...
        self.mean =  nn.Sequential(
            nn.Conv2d(representation_size*4, representation_size*4, 5, stride=1, padding=2)
        )
...
class Decoder_st(nn.Module):
    def __init__(self, input_size, representation_size):
        super(Decoder_st, self).__init__()
        self.input_size = input_size
        self.representation_size = representation_size
        dim = representation_size[0] * representation_size[1] * representation_size[2]
    ...
    def forward(self, code):
        ...
        output = self.deconv1(code, output_size=(bs, 256, 16, 16))
        output = self.act1(output)
        output = self.deconv2(output, output_size=(bs, 128, 32, 32))
        output = self.act2(output)
...
class VAE_Generator_st(nn.Module):
    def __init__(self, input_channels, hidden_size, representation_size=(256, 8, 8)):
        super(VAE_Generator_st, self).__init__()
        self.input_channels = input_channels
        self.hidden_size = hidden_size
        self.representation_size = representation_size
        
        self.encoder = Encoder_st(input_channels, hidden_size)
        self.decoder = Decoder_st(hidden_size, representation_size)
    ...
    def forward(self, x):
        ...
        reparametrized_noise = torch.randn_like(std)

...
class Decoder(nn.Module):
    def __init__(self, input_size, representation_size):
        ...
        self.deconv5 = nn.ConvTranspose2d(64, 3, 5, stride=1, padding=2)
            # 3 x 128 x 128
    ...
    def forward(self, code):
        ...
        output_Q50 = self.deconv4(output, output_size=(bs, 3, 64, 64))
        output_Q15 = self.deconv5(output, output_size=(bs, 3, 64, 64))

       
        return output_Q50,output_Q15
...
class VAE_Generator(nn.Module):
    ...
    def forward(self, x):
        ...
        Q50_images, Q15_images= self.decoder(reparametrized_noise)
        
        return mean, logvar, Q50_images, Q15_images
...
class Decoder_var(nn.Module):
    ...
    def forward(self, code):
        ...
        return self.activation2(output_Q50),self.activation2(output_Q15)



class VAE_Generator_var(nn.Module):
    def __init__(self, input_channels, hidden_size, representation_size=(256, 8, 8)):
        super(VAE_Generator_var, self).__init__()
        self.input_channels = input_channels
        self.hidden_size = hidden_size
        self.representation_size = representation_size
        
        self.encoder = Encoder_var(input_channels, hidden_size)
        self.decoder = Decoder_var(hidden_size, representation_size)

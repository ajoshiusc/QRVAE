...
def beta_loss_function(recon_x, x,z,beta,zeta,Q):
...
def train(epoch, beta_val):
    ...
    for batch_idx, (data, data_lab) in enumerate(train_loader):
        ...
        loss_Q1,z_loss = beta_loss_function(recon_batch[:,0,:], data, z, beta_val, zeta_val,0.15)
        loss_Q2,z_loss = beta_loss_function(recon_batch[:,1,:], data, z, beta_val, zeta_val,0.5)
        loss_Q3,z_loss = beta_loss_function(recon_batch[:,2,:], data, z, beta_val, zeta_val,0.85)
        loss=(loss_Q1+loss_Q2+loss_Q3)/3+reg_weight*regularize_loss+z_loss
        loss.backward()
        ...
        for i, (data, data_lab) in enumerate(test_loader):
            ...
            if i == 0:
                ...
                comparison = torch.cat([
                    ...
                    (torch.abs(recon_batch[:,2,:]-recon_batch[:,1,:])/2).view(len(recon_batch), 1, 28, 28)[samp]
